      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-16 space-y-6 lg:mt-20">
          {services.map((service, index) => (
            <article
              key={service.id}
              className="group overflow-hidden rounded-2xl border border-[var(--border)] bg-white transition-all hover:shadow-lg"
            >
              <div className={`grid gap-0 lg:grid-cols-2 ${index % 2 === 1 ? "lg:grid-flow-dense" : ""}`}>
                ...
                <div className={`relative min-h-[300px] bg-gradient-to-br ${service.accent} p-6 lg:min-h-[400px] lg:p-8 ${index % 2 === 1 ? "lg:col-start-2" : ""}`}>
                  <div className="absolute inset-6 lg:inset-8">
                    <div className={`h-3/5 rounded-xl bg-gradient-to-br ${service.panel} opacity-80`} />
                    <div className="absolute right-0 bottom-0 w-2/3 rounded-xl border border-white/40 bg-white/60 p-4 backdrop-blur-sm">
                      <p className="text-xs font-medium tracking-widest text-[var(--muted)] uppercase">
                        Visual Direction
                      </p>
